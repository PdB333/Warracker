(function() {
    ...
    function createStatusChart(stats) {
        ...
        try {
            ...
            const activeLabel = t('warranties.active', 'Active');
            const expiringSoonLabel = t('warranties.expiring_soon', 'Expiring Soon');
            const expiredLabel = t('warranties.expired', 'Expired');
    ...
    function createTimelineChart(timelineData) {
        ...
        try {
            ...
            const timelineLabel = t('status.expiration_timeline', 'Expiration Timeline');
    ...
    function filterAndSortWarranties() {
        ...
        if (!allWarranties || allWarranties.length === 0) {
            ...
            const noWarrantiesMessage = t('status.recent_expirations_empty', 'No recently expired or expiring warranties.');
        ...
        if (displayWarranties.length === 0) {
            const colspan = isGlobalView ? 5 : 4;
            const noMatchMessage = t('messages.no_results', 'No results found');
        ...
        displayWarranties.forEach(warranty => {
            ...
            const lifetimeText = t('warranties.lifetime', 'Lifetime');
            const naText = t('warranties.na', 'N/A');
            let rowHTML = `
