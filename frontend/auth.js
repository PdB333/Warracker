            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (data.valid && data.user && data.user.id) {
                        ...
                        console.warn('[Auth.js] Token validation returned invalid user data. Clearing auth data.');
                        this.clearAuthData();
                    }
                } else if (response.status === 401 || response.status === 403) {
                    console.warn(`[Auth.js] Token rejected by API (status: ${response.status}). Clearing auth data.`);
                    this.clearAuthData();
                } else {
                    console.warn(`[Auth.js] Token validation API returned ${response.status}. Preserving local auth state.`);
                }
            } catch (error) {
                console.error('[Auth.js] Error validating token / fetching user info. Preserving local auth state:', error);
            }
        } else {
