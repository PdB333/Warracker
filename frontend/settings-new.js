...
function initLanguageSelector() {
    ...
    languageSelect.addEventListener('change', async function() {
        ...
        persistPreferredLanguage(selectedLanguage);

        try {
            showToast('Changing language...', 'info');
            const result = await applyLanguageChange(selectedLanguage);
            ...
            if (result.changed) {
                ...
                console.warn('Language change function unavailable, applying on reload.');
                showToast('Language will be applied after reload.', 'warning');
                setTimeout(() => {
...
function persistPreferredLanguage(language) {
    ...
    // i18n loader checks cookie first, so keep cookie and localStorage aligned.
    document.cookie = `lang=${encodeURIComponent(language)};path=/;max-age=${365 * 24 * 60 * 60}`;
}
...
function getLanguageChangeFunction() {
    if (window.i18n && typeof window.i18n.changeLanguage === 'function') {
        return window.i18n.changeLanguage.bind(window.i18n);
    }
    if (window.i18next && typeof window.i18next.changeLanguage === 'function') {
        return window.i18next.changeLanguage.bind(window.i18next);
    }
    return null;
}
...
function waitForI18nReady(timeoutMs = 2500) {
    return new Promise((resolve) => {
        ...
        const onReady = () => finish(!!getLanguageChangeFunction());
        window.addEventListener('i18nReady', onReady, { once: true });

        const timeoutId = setTimeout(() => finish(!!getLanguageChangeFunction()), timeoutMs);
    });
}

async function applyLanguageChange(selectedLanguage) {
    if (!selectedLanguage) return { changed: false, reason: 'missing_language' };

    const currentLang = getCurrentLanguageSafe();
    if (selectedLanguage === currentLang) {
        return { changed: false, reason: 'same_language' };
    }

    let changeLanguageFn = getLanguageChangeFunction();
    if (!changeLanguageFn) {
        await waitForI18nReady();
        changeLanguageFn = getLanguageChangeFunction();
    }

    if (!changeLanguageFn) {
        return { changed: false, reason: 'i18n_not_ready' };
    }

    await changeLanguageFn(selectedLanguage);
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    await window.auth.checkAuthState(true);
    ...
    const debugCurrentUser = window.auth.getCurrentUser ? window.auth.getCurrentUser() : null;
...
async function savePreferences() {
    ...
    persistPreferredLanguage(preferencesToSave.preferred_language); // Save language preference
    ...
    // Try saving to API
        ...
        try {
            ...
            if (response.ok) {
                ...
                // If language was changed, apply it immediately when possible, otherwise apply after reload.
                if (preferencesToSave.preferred_language) {
                    const currentLang = getCurrentLanguageSafe();
                    if (preferencesToSave.preferred_language !== currentLang) {
                        ...
                        try {
                            const result = await applyLanguageChange(preferencesToSave.preferred_language);
                            if (!result.changed) {
                                console.warn('Language change function unavailable during save, applying on reload.');
                                showToast('Language will be applied after reload.', 'warning');
                                // Force a reload only if immediate language switch is not available.
                                setTimeout(() => {
                                    window.location.reload();
                                }, 1000);
                            }
...
async function deleteAccount() {
    ...
    try {
        ...
        if (response.ok) {
            // Clear auth data locally without issuing a second logout request.
            if (window.auth && typeof window.auth.clearAuthData === 'function') {
                window.auth.clearAuthData();
                if (typeof window.auth.updateUIBasedOnAuthState === 'function') {
...
async function loadSiteSettings() {
    ...
    try {
        ...
        if (oidcAdminGroupInputElem) {
            const oidcAdminGroupValue = settings.oidc_admin_group || settings.admin_oidc_group || '';
            console.log('[OIDC Settings] Found oidcAdminGroupInputElem. Setting value to:', oidcAdminGroupValue);
            if (!settings.oidc_admin_group && settings.admin_oidc_group) {
                console.warn('[OIDC Settings] Loaded legacy key admin_oidc_group; it will be saved as oidc_admin_group.');
            }
            oidcAdminGroupInputElem.value = oidcAdminGroupValue;
        } else {
