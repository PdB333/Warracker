...
RUN apt-get update && \
    ...
    if ! apt-cache show "${CURL_DEV_PKG}" >/dev/null 2>&1; then CURL_DEV_PKG="libcurl4-openssl-devt64"; fi && \
...
RUN apt-get update && \
    CURL_RUNTIME_PKG="libcurl4" && \
    if ! apt-cache show "${CURL_RUNTIME_PKG}" >/dev/null 2>&1; then CURL_RUNTIME_PKG="libcurl4t64"; fi && \
    apt-get install -y --no-install-recommends \
        nginx \
        supervisor \
        postgresql-client \
