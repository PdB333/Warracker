...
def generate_token(user_id, session_token):
    """Generate a JWT token for the user and bind it to a server-side session."""
    payload = {
...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            ...
            current_app.logger.warning(f"Token missing valid subject for: {request.path}")
            return jsonify({'message': 'Invalid or expired token!'}), 401

        session_token = payload.get('sid')
        if not session_token:
            current_app.logger.warning(f"Token missing session binding for: {request.path}")
            return jsonify({'message': 'Session is invalid or has expired!'}), 401
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                if not cur.fetchone():
                    current_app.logger.warning(f"Revoked or unknown session used for: {request.path}")
                    return jsonify({'message': 'Session is invalid or has expired!'}), 401
