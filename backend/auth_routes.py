...
def register():
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute('SELECT id FROM users WHERE username = %s OR LOWER(email) = LOWER(%s)', (username, email))
            ...
            # Generate token bound to the persisted session
            token = generate_token(user_id, session_token)
...
def login():
    ...
    try:
        ...
        conn = db_handler.get_db_connection()
        with conn.cursor() as cur:
            # Check if user exists - include oidc_sub to detect SSO-managed accounts
            cur.execute(
                'SELECT id, username, email, password_hash, is_active, is_admin, oidc_sub FROM users WHERE username = %s OR LOWER(email) = LOWER(%s)',
                (username, username)
            )
            user = cur.fetchone()
            ...
            oidc_managed = user[6] is not None

            if not password_hash:
                if oidc_managed:
                    return jsonify({'message': 'This account uses SSO login. Please sign in with your SSO provider.'}), 401
                return jsonify({'message': 'Invalid username or password!'}), 401
...
def logout():
    ...
    try:
        ...
        session_token = request.user.get('session_token')

        conn = db_handler.get_db_connection()
        with conn.cursor() as cur:
            if session_token:
                cur.execute('DELETE FROM user_sessions WHERE user_id = %s AND session_token = %s', (user_id, session_token))
            else:
    ...
    try:
        ...
        return jsonify({
            ...
            'user': {
                ...
                'oidc_managed': request.user.get('oidc_managed', False)
            },
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            try:
                ...
                columns = ['id', 'username', 'email', 'first_name', 'last_name', 'is_admin', 'is_owner', 'oidc_sub']
            except Exception as e:
        ...
        # Turn oidc_sub into a boolean for the API response
        user_info['oidc_managed'] = bool(user_info.pop('oidc_sub', None)) if 'oidc_sub' in user_info else False
...
def delete_account():
    ...
    try:
        ...
        try:
            ...
            # Preserve the user's warranties by detaching them from the deleted account
            cursor.execute("UPDATE warranties SET user_id = NULL WHERE user_id = %s", (user_id,))
            warranties_detached = cursor.rowcount
            current_app.logger.info(f"Detached {warranties_detached} warranties from user {user_id}")
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            if not current_password_hash:
                return jsonify({'message': 'Password changes are not available for SSO-managed accounts.'}), 400
...
def send_email_change_verification_email(recipient_email, verify_link):
    ...
    try:
        ...
        server.sendmail(sender_email, recipient_email, msg.as_string())
        server.quit()
        current_app.logger.info(f"Email change verification email sent successfully to {recipient_email}")
        return True
    except Exception as e:
        current_app.logger.error(f"Error sending email change verification email: {e}")
        return False
...
def request_email_change():
    ...
    try:
        ...
        # Check if the new email is already in use by another user
        cur.execute("SELECT id FROM users WHERE LOWER(email) = LOWER(%s) AND id != %s", (new_email, current_user_id))
        if cur.fetchone():
            return jsonify({'message': 'This email address is already in use by another account.'}), 409
        ...
        # Keep one active token per user to avoid confusion with multiple pending requests.
        cur.execute("DELETE FROM email_change_tokens WHERE user_id = %s", (current_user_id,))
        cur.execute(
            "INSERT INTO email_change_tokens (user_id, new_email, token, expires_at) VALUES (%s, %s, %s, %s)",
            (current_user_id, new_email, verification_token, expires_at)
        )
        conn.commit()
...
def verify_email_change():
    """Verify pending email change using a one-time token."""
    data = request.get_json() or {}
    ...
    try:
        ...
        cur.execute(
            "SELECT user_id, new_email, expires_at FROM email_change_tokens WHERE token = %s",
            (verification_token,)
        )
        ...
        user_id, new_email, expires_at = token_data
        if expires_at < datetime.now(UTC):
            cur.execute("DELETE FROM email_change_tokens WHERE token = %s", (verification_token,))
            conn.commit()
        ...
        cur.execute("UPDATE users SET email = %s WHERE id = %s", (normalize_email(new_email), user_id))
        cur.execute("DELETE FROM email_change_tokens WHERE user_id = %s", (user_id,))
        conn.commit()
        return jsonify({'message': 'Email address verified and updated successfully.'}), 200
    except Exception as e:
        if conn:
            conn.rollback()
        current_app.logger.error(f"Error verifying email change: {e}")
        return jsonify({'message': 'An unexpected error occurred.'}), 500
    finally:
        if cur:
            cur.close()
