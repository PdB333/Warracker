...
def _get_orphan_warranty_recipient(conn):
    ...
    Priority: active owner, then first active admin.
    """
    cur = None
    try:
        cur = conn.cursor()
...
def get_expiring_warranties(get_db_connection, release_db_connection):
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            if orphan_recipient:
                with conn.cursor() as orphan_cur:
                    orphan_days = 30
                    orphan_cur.execute("""
                        SELECT COALESCE(expiring_soon_days, 30)
                        FROM user_preferences
                        WHERE user_id = %s
                    """, (orphan_recipient['user_id'],))
                    orphan_days_row = orphan_cur.fetchone()
                    if orphan_days_row and orphan_days_row[0] is not None:
                        orphan_days = int(orphan_days_row[0])
                    ...
                    if orphan_rows:
                        logger.info(
                            f"Included {len(orphan_rows)} orphan warranties for notifications "
                            f"via fallback recipient {orphan_recipient['email']} "
                            f"(user_id={orphan_recipient['user_id']})"
                        )
            else:
                logger.warning("No active owner/admin available for orphan warranty notifications.")
...
def should_run_scheduler():
    ...
    # Robust cross-process guard: ensure only one process can run scheduler.
    # This protects against duplicated schedulers when worker-id detection
    # is unavailable/misordered in some gunicorn startup paths.
    global _scheduler_process_lock
    if _scheduler_process_lock is None:
        try:
            import fcntl
            lock_file = open('/tmp/warracker_scheduler.lock', 'w')
            fcntl.flock(lock_file.fileno(), fcntl.LOCK_EX | fcntl.LOCK_NB)
            _scheduler_process_lock = lock_file
            logger.info("✅ Scheduler process lock acquired.")
        except Exception:
            logger.info("ℹ️ Scheduler process lock already held by another process.")
            return False
