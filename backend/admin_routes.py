...
def delete_user(user_id):
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute('UPDATE warranties SET user_id = NULL WHERE user_id = %s', (user_id,))
            warranties_detached = cur.rowcount
            logger.info(f"Detached {warranties_detached} warranties from user {user_id}")
...
def get_site_settings():
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            # Backward compatibility: migrate legacy OIDC admin group key to the current key.
            legacy_oidc_admin_group = raw_settings.get('admin_oidc_group')
            if legacy_oidc_admin_group and not raw_settings.get('oidc_admin_group'):
                raw_settings['oidc_admin_group'] = legacy_oidc_admin_group
                cur.execute(
                    """
                    ...
                    """,
                    ('oidc_admin_group', legacy_oidc_admin_group)
                )
                needs_commit = True

            # First, add all existing settings from the database
            for key, value in raw_settings.items():
                # Hide legacy alias from API responses once normalized.
                if key == 'admin_oidc_group':
                    continue
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            for key, value in data.items():
                # Normalize legacy key for backward compatibility
                if key == 'admin_oidc_group':
